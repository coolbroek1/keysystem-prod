exports.handler = async (event) => {
  ...
  try {
    ...
    let payload = null;
    if (req.op === 'issue')         payload = { action:'admin_issue_key',   hwid:req.hwid, adminSecret:ADMIN_SECRET };
    else if (req.op === 'lookup_hwid') payload = { action:'admin_lookup_hwid', hwid:req.hwid, adminSecret:ADMIN_SECRET };
    else if (req.op === 'reset_hwid')  payload = { action:'admin_reset_hwid',  hwid:req.hwid, adminSecret:ADMIN_SECRET };
    else if (req.op === 'delete_key')  payload = { action:'admin_delete_key',  key:req.key,  adminSecret:ADMIN_SECRET };
